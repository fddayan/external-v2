...
import { CurvedContainer, Display1, Display2, Headline2 } from "./styles";
...
import { SecondaryButton, LinkButton } from "./styles";
...
const PanelCTAContainer = styled.div`
  display: flex;
  gap: 24px;
  ${mediaQueriesMax[0]} {
    display: none;
...
const StickyCarousel = ({ content, openCalendly }) => {
  ...
  return (
    <CurvedContainer css={{ paddingBlock: 100, marginBottom: 100 }}>
      <Container>
        ...
        >
          <Translate path="directus.page_schoolleader_2024.carousel_heading" />
        </Display1>
        ...
        <PanelContainer>
          ...
          {content.map((panel, index) => (
            <Panel key={index} ref={panelRefs.current[index]}>
              <div className="image-container" css={{ backgroundImage: `url(${panel.mobileSrc})` }}></div>
              <div className="content-container">
                <DetailText css={{ color: "#ffffff" }}>{panel.detail_text}</DetailText>
                <Display2 css={{ color: "#ffffff" }}>{panel.heading}</Display2>
                <Headline2 css={{ color: "#ffffff" }}>{panel.paragraph}</Headline2>
                <PanelCTAContainer>
                  <SecondaryButton
                    kind="secondary"
                    className="secondary"
                    onClick={() => openCalendly("test_stickypanel_" + index)}
                  >
                    <Translate path="directus.page_schoolleader_2024.carousel_primary_cta_copy" />
                  </SecondaryButton>
                  <LinkButton kind="link" className="link" onClick={openSignupModal}>
                    <Translate path="directus.page_schoolleader_2024.carousel_secondary_cta_copy" />
                  </LinkButton>
                </PanelCTAContainer>
              </div>
            </Panel>
        ...
        <MobileCTAContainer>
          <Button kind="secondary" onClick={openCalendly}>
            <Translate path="directus.page_schoolleader_2024.carousel_primary_cta_copy" />
          </Button>
        </MobileCTAContainer>
      </Container>
      <LinkButton kind="link" className="secondary-cta" onClick={openSignupModal}>
        <Translate path="directus.page_schoolleader_2024.carousel_secondary_cta_copy" />
      </LinkButton>
    </CurvedContainer>
  );
